...
// We need to be able to quickly multiply sets of algebra elements
// Could also make a version to multiply L with L and halve the time.
intrinsic BulkMultiply(L::SeqEnum[AlgGenElt], M::SeqEnum[AlgGenElt]) -> SeqEnum
  {
  Given two sequences of algebra elements, return the sequence of sequences of elements l*m, where rows correspond to L and columns correspond to M.
  
  Note: this is much faster than just multiplying these elements together individually.  
  }
  A := Universe(L);
  require A eq Universe(M): "The elements must be from the same algebra.";
  require IsCommutative(A): "This is currently only implemented for commutative algebras.";
  
  // I think we want to maximise the number of matrices as matrix multiplication is quick, but building adjoints takes time.  (For A6 121-dim, 19 adjoints takes 6 sec, but applying all these to 19 vectors takes 0.15 sec.  However, this might be different for smaller algebras.)
  
  // Assume commutativity
  if #L lt #M then
    out := BulkMultiply(M, L);
    return [ [out[j,i] : j in [1..#M] ] : i in [1..#L]];
  end if;
  
  // So we have #M \leq #L
    
  adjs := [ AdjointMatrix(m) : m in M];
  Lmat := Matrix([Vector(l) : l in L]);
  prods := [ [ A | v : v in Rows(Lmat*ad)] : ad in adjs];
  
  // swap as we should have rows corresponding to L and columns to M - this shouldn't take time
  return [ [prods[j,i] : j in [1..#M]] : i in [1..#L]];
end intrinsic;

intrinsic BulkMultiply(L::SeqEnum[DecAlgElt], M::SeqEnum[DecAlgElt]) -> SeqEnum
  {
  Given two sequences of decomposition algebra elements, return the sequence of sequences of elements l*m, where rows correspond to L and columns correspond to M.
  
  Note: this is much faster than just multiplying these elements together individually.  
  }
  A := Universe(L);
  require A eq Universe(M): "The elements must be from the same decomposition algebra.";
  
  Aalg := Algebra(A);
  
  prods := BulkMultiply([Aalg | Vector(l) : l in L], [Aalg | Vector(m) : m in M]);
  return [ [ A| a : a in R] : R in prods];
end intrinsic;
...
intrinsic 'subset'(A::DecAlg, B::DecAlg) -> BoolElt
  {
  Returns whether A is a subalgebra of B.
  }
  Aalg := Algebra(A);
  Balg := Algebra(B);
  
  return Aalg subset Balg;
  // FIX THIS PROPERLY
  // return IsSubalgebra(A, B);
end intrinsic;

/*
intrinsic IsSubalgebra(A::DecAlg, B::DecAlg) -> BoolElt, Map
  {
  Returns whether A is a subalgebra of B and if so, returns the inclusion map.
  }
  Aalg := Algebra(A);
  Balg := Algebra(B);
  
  return Balg subset Aalg;
  /*
  // FIX THIS PROPERLY
  
  if not Aalg subset Balg then
    return false, _;
  end if;
  /*
end intrinsic;
...
*/
// MAGMA BUG in subalgebras for nonassociative algebras

intrinsic Subalgebra(A::AlgGen, L::.) -> AlgGen, Map
  {
  There is a Magma bug in constructing subalgebras.  This is a workaround.
  
  Returns the subalgebra of A generated by L.  L must be either: an element of A, a set or sequence of elements of A, a subalgebra or ideal of A, or a set or sequence of subalgebras or ideals of A.
  }
  V := VectorSpace(A);
  if IsEmpty(L) then
    B := [ V | ];
  elif Type(L) eq AlgGenElt and IsCoercible(A, L) then
    B := [ V!L ];
  elif ISA(Type(L), {MakeType("Set"), MakeType("SeqEnum")}) and IsCoercible(A, L[1]) then
    B := [ V | V!x : x in L];
  else
    error "L is not given in the correct form.";
  end if;
  ...
  Uold_bas := [];
  U_bas := ChangeUniverse(Basis(U), A);
  Unew_bas := U_bas;
  
  while #Unew_bas ne 0 and Dimension(U) ne Dimension(V) do
    prods := BulkMultiply(U_bas, Unew_bas);
    
    // Update the subspaces and bases
    Uold := U;
    U +:= sub<V | ChangeUniverse(Flat(prods), V)>;
    Uold_bas := U_bas;
    U_bas := ChangeUniverse(Basis(U), A);
    Unew_bas := ExtendBasis(Uold, U)[Dimension(Uold)+1..Dimension(U)];
    Unew_bas := ChangeUniverse(Unew_bas, A);
  end while;
...
intrinsic Subalgebra(A::DecAlg, L::.) -> AlgGen//, Map
  {
  Returns the subalgebra of A generated by L.  L must be either: an element of A, a set or sequence of elements of A, a subalgebra or ideal of A, or a set or sequence of subalgebras or ideals of A.
  }
  Aalg := Algebra(A);
  ...
  if IsEmpty(L) then
    LL := [];
  elif Type(L) eq DecAlgElt then
    LL := [ Aalg!Eltseq(L[1]) ];
  elif ISA(Type(L), {MakeType("Set"), MakeType("SeqEnum")}) and IsCoercible(A, L[1]) then
    LL := [ Aalg | Aalg!Eltseq(x) : x in L];
  end if;
  ...
  Balg, inc_alg := Subalgebra(Aalg, LL);

  return Balg, inc_alg;
end intrinsic;

intrinsic SubConstructor(A::DecAlg, L::.) -> DecAlg//, Map
  {
  ...
  If any decompositions which is axial has its axis in the subalgebra, then it remains an axial decomposition.
  }
  // Also returns the inclusion homomorphism f:B -> A.
  Aalg := Algebra(A);
  
  B, inc := Subalgebra(A, L);
  ...
  for k in IndexSet(A) do
    ...
    if &+[Dimension(U) : U in partsnew] ne Dimension(Anew) then
      continue k;
    end if;
    
    // If D is axial then check to see whether the axis is in the decomposition
    if IsAxial(D) and Vector(Axis(D)) in W then
      Dnew := AxialDecomposition(Anew, partsnew, Vector(Axis(D))@Vincinv);
    else
      Dnew := Decomposition(Anew, partsnew);
    end if;
    Append(~Dsnew, <k,Dnew>);
  end for;
...
intrinsic '@'(x::DecAlgElt, phi::Map) -> DecAlgElt
  {
  Returns the image of x under phi.
  }
  require x in Domain(phi): "The element is not in the doamin of the map.";
...
intrinsic Print(D::Dec)
  ...
  printf "Decomposition of a %o-dimensional algebra into %o parts of dimensions %o", 
    Dimension(Algebra(D)), NumberOfParts(D), 
...
intrinsic Parts(D::Dec) -> SeqEnum
  ...
  el := Elements(FusionLaw(Algebra(D)));
...
intrinsic Part(D::Dec, x::FusLawElt) -> ModTupRng
  ...
  if not IsDefined(D`parts, x) then
    D`parts[x] := sub<VectorSpace(Algebra(D))|>;
  end if;
  return D`parts[x];
end intrinsic;
...
intrinsic Label(D::Dec) -> .
  ...
  require IsAttached(D): "D is not attached.";
  P := Algebra(D);
...
intrinsic Decomposition(A::DecAlg, S::[ModTupRng]) -> Dec
  {
  Given a set of subspaces S of a decomposition algebra A, creates a Decomposition for A with respect to S.
  }
